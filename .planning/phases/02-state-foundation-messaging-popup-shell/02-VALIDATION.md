...
- **Before `/gsd-verify-work`:** Full suite must be green AND `apps/extension/.output/chrome-mv3/registry.json` must exist with all 8 schema fields
...
- [ ] `apps/extension/src/content/abort-signal.test.ts` — AbortSignal honored on cleanup; `AbortError` not escalated
...
| `registry.json` artifact present | BLD-03 | Build emission verified by unit, but production presence on disk is a phase-gate fact | `cat apps/extension/.output/chrome-mv3/registry.json` shows valid JSON array with all 8 fields per experiment (`id, author, name, description, scope, world, chunkPath, tweaks`). |
